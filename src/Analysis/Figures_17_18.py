...
# Load in Raw Data
#----------------------------------------------
with open("../../data/Optimal IPHROS No RO/no_RO_turbo_dicts.pkl", "rb") as f:
    dict_turbo_RO_convergence, dict_LP_efficiencies_convergence, dict_MILP_post_processing, dict_big_convergence = pickle.load(f)

x_MILP_season0 = pd.read_csv('../../data/Optimal IPHROS No RO/x_MILP_season0_No_RO.csv',header=None).to_numpy()
x_MILP_season1 = pd.read_csv('../../data/Optimal IPHROS No RO/x_MILP_season1_No_RO.csv',header=None).to_numpy()
x_MILP_season2 = pd.read_csv('../../data/Optimal IPHROS No RO/x_MILP_season2_No_RO.csv',header=None).to_numpy()
x_MILP_season3 = pd.read_csv('../../data/Optimal IPHROS No RO/x_MILP_season3_No_RO.csv',header=None).to_numpy()
...
#%% Read in the reservoir levels for the optimal IPHROS, adjust V_res arrays for the no_RO design

reservoir_level_dict_optimal_IPHROS = pd.read_pickle("../../data/Optimal IPHROS/reservoir_levels_dict.pkl")

V_res_spring_IPHROS = reservoir_level_dict_optimal_IPHROS['spring']
V_res_summer_IPHROS = reservoir_level_dict_optimal_IPHROS['summer']
V_res_fall_IPHROS = reservoir_level_dict_optimal_IPHROS['fall']
V_res_winter_IPHROS = reservoir_level_dict_optimal_IPHROS['winter']

# Add the initial reservoir volume to the arrays (this was already done for the IPHROS ones)
...
#%% Make a reservoir plot

x_array = np.linspace(0,167,168)
x_array_res = np.linspace(-1,167,169)

fig, axes = plt.subplots(1,2,figsize=(10,5))#(figsize=(8, 5))
axes[0].plot(x_array_res, V_res_spring_IPHROS, linewidth=2, label=r'$Spring$')  
axes[0].plot(x_array_res, V_res_summer_IPHROS, linewidth=2, label=r'$Summer$')
axes[0].plot(x_array_res, V_res_fall_IPHROS, linewidth=2, label=r'$Fall$')
axes[0].plot(x_array_res, V_res_winter_IPHROS, linewidth=2, label=r'$Winter$')
axes[0].axhline(y=V_res_min,linewidth=2,linestyle='--',color='black', alpha=0.5, label=r'$Reservoir \; Limits$')
axes[0].axhline(y=V_res_max,linewidth=2,linestyle='--',color='black', alpha=0.5)
axes[0].set_xlabel(r'$Time \; [hr]$', fontsize=14)
axes[0].set_ylabel(r'$V_{res} \; [m^3]$', fontsize=14)
axes[0].set_title(r"$(a)$",fontsize=14)
axes[0].set_xticks(np.arange(0, max(x_array)+2, 24))  # ticks every 24

axes[1].plot(x_array_res, V_res_spring, linewidth=2)  
axes[1].plot(x_array_res, V_res_summer, linewidth=2)
axes[1].plot(x_array_res, V_res_fall, linewidth=2)
axes[1].plot(x_array_res, V_res_winter, linewidth=2)
axes[1].axhline(y=V_res_min,linewidth=2,linestyle='--',color='black', alpha=0.5)
axes[1].axhline(y=V_res_max,linewidth=2,linestyle='--',color='black', alpha=0.5)
axes[1].set_xlabel(r'$Time \; [hr]$', fontsize=14)
axes[1].set_ylabel(r'$V_{res} \; [m^3]$', fontsize=14)
axes[1].set_title(r"$(b)$",fontsize=14)
axes[1].set_xticks(np.arange(0, max(x_array)+2, 24))  # ticks every 24
...
plt.tight_layout(rect=[0, 0.07, 0.97, 1])
