# -*- coding: utf-8 -*-
"""
Created on Wed Mar 25 13:19:54 2026
...
#----------------------------------------------
# Actual production plots
#----------------------------------------------
hr_array = np.linspace(0,(24*7-1),(24*7))+1

fig, axes = plt.subplots(2, 2, figsize=(10, 8), sharex=True, sharey=True)

# Spring
ax_spring = axes[0,0]
ax_spring.fill_between(hr_array,(P_out_PSH_spring/1000),label=r'$PSH \; Generation$')
ax_spring.fill_between(hr_array,-(P_in_PSH_spring/1000),label=r'$IPHROS \; Consumption$')
ax_spring.set_title(r'$Spring$',fontsize=14)
ax_spring.set_xticks(np.arange(0, max(hr_array)+2, 24))  # ticks every 24

ax_spring_a = ax_spring.twinx()
ax_spring_a.step(hr_array, V_dot_fwRO_spring, 
                 color="green",
                 linewidth=1.2,
                 linestyle="--",
                 alpha=0.8,
...
# Summer
ax_summer = axes[0,1]
ax_summer.fill_between(hr_array,(P_out_PSH_summer/1000),label=r'$PSH \; Generation$')
ax_summer.fill_between(hr_array,-(P_in_PSH_summer/1000),label=r'$IPHROS \; Consumption$')
ax_summer.set_title(r'$Summer$',fontsize=14)
ax_summer.set_xticks(np.arange(0, max(hr_array)+2, 24))  # ticks every 24

ax_summer_a = ax_summer.twinx()
ax_summer_a.step(hr_array, V_dot_fwRO_summer, 
                 color="green",
                 linewidth=1.2,
                 linestyle="--",
                 alpha=0.8,
...
# Fall
ax_fall = axes[1,0]
ax_fall.fill_between(hr_array,(P_out_PSH_fall/1000),label=r'$PSH \; Generation$')
ax_fall.fill_between(hr_array,-(P_in_PSH_fall/1000),label=r'$IPHROS \; Consumption$')
ax_fall.set_title(r'$Fall$',fontsize=14)
ax_fall.set_xticks(np.arange(0, max(hr_array)+2, 24))  # ticks every 24

ax_fall_a = ax_fall.twinx()
ax_fall_a.step(hr_array, V_dot_fwRO_fall, 
               color="green",
               linewidth=1.2,
               linestyle="--",
               alpha=0.8,
...
# Winter
ax_winter = axes[1,1]
ax_winter.fill_between(hr_array,(P_out_PSH_winter/1000),label=r'$PSH \; Generation$')
ax_winter.fill_between(hr_array,-(P_in_PSH_winter/1000),label=r'$IPHROS \; Consumption$')
ax_winter.set_title(r'$Winter$',fontsize=14)
ax_winter.set_xticks(np.arange(0, max(hr_array)+2, 24))  # ticks every 24

ax_winter_a = ax_winter.twinx()
ax_winter_a.step(hr_array, V_dot_fwRO_winter, 
                 color="green",
                 linewidth=1.2,
                 linestyle="--",
                 alpha=0.8,
...
# Shared Axis Labels
fig.supxlabel(r"$Time \; [hr]$",fontsize=14, y=0.075)
fig.supylabel(r"$Energy \; Output \; [MWh]$",fontsize=14)

fig.text(
    0.97, 0.5,
    r"$RO \; Permeate \; Flow \; [m^3/hr]$",
    va='center',
    rotation=90,
    fontsize=14
)
...
handles_part1, labels_part1 = ax_spring.get_legend_handles_labels()
handles_part2, labels_part2 = ax_spring_a.get_legend_handles_labels()
...
plt.tight_layout(rect=[0, 0.06, 0.97, 1])  # leave space for legend
...
#%% Profits analysis
#----------------------------------------------
# Electricity and Water Profits
#----------------------------------------------
# Water Shit
water_price = 1.97 # Euros/m^3
    
V_fwRO_spring_week = np.sum(V_dot_fwRO_spring)
V_fwRO_summer_week = np.sum(V_dot_fwRO_summer)
V_fwRO_fall_week = np.sum(V_dot_fwRO_fall)
V_fwRO_winter_week = np.sum(V_dot_fwRO_winter)

water_profit_spring = water_price*V_fwRO_spring_week
water_profit_summer = water_price*V_fwRO_summer_week
water_profit_fall = water_price*V_fwRO_fall_week
water_profit_winter = water_price*V_fwRO_winter_week

average_weekly_water_profit = np.mean([water_profit_spring,water_profit_summer,water_profit_fall,water_profit_winter])
...
P_net_spring = P_out_PSH_spring-P_in_PSH_spring
P_net_summer = P_out_PSH_summer-P_in_PSH_summer
P_net_fall = P_out_PSH_fall-P_in_PSH_fall
P_net_winter = P_out_PSH_winter-P_in_PSH_winter

elec_profit_array_spring = energy_prices_spring*P_net_spring
elec_profit_array_summer = energy_prices_summer*P_net_summer
elec_profit_array_fall = energy_prices_fall*P_net_fall
elec_profit_array_winter = energy_prices_winter*P_net_winter


elec_profit_spring = np.sum(elec_profit_array_spring)
elec_profit_summer = np.sum(elec_profit_array_summer)
elec_profit_fall = np.sum(elec_profit_array_fall)
elec_profit_winter = np.sum(elec_profit_array_winter)

average_weekly_electricity_profit = np.mean([elec_profit_spring,elec_profit_summer,elec_profit_fall,elec_profit_winter])
